...
class AuthenticateAdmin
{
    ...
    {
        //查找出所有的菜单
        $table = Permission::orderBy('fid', 'asc')->orderBy('sort', 'desc')->get()->toArray();
        $newData = [];
        $user = Auth::user();

          //dd(Route::currentRouteName());
        $currentPermission = Permission::where('name', Route::currentRouteName())->select('id', 'fid')->first();
                ...
                foreach ($table as $key => $data) {
                    ...
                    if ($user->is_super || in_array($data['id'], $permissions)) {
                        if (0 == $parentId) {
                            ...
                            if (isset($refer[$parentId])&&$data['is_menu']==1) {
