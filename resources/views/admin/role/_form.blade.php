                ...
                @if(isset($v['_child']) && !empty($v['_child']))
                    @foreach($v['_child'] as $vv)
                        <div class="col-sm-3 ">
                            ...
                            @if(isset($vv['_child']) && !empty($vv['_child']))
                                <div class="subs-permissions col-md-11 col-md-offset-1" style="display: block;">
                                @foreach($vv['_child'] as $vvv)
                                    <div class="col-sm-5">
                                        <label><input type="checkbox" name="permissions[]" value="{{ $vvv['id'] }}" class="subs-permission-checkbox" {{ in_array($vvv['id'], $perms) ? "checked" : "" }} >&nbsp;&nbsp;{{ $vvv['display_name'] }}
                                        </label>
        ...
        $(document).ready(function(){
            ...
            $(".sub-permission-checkbox").on('ifChecked ifUnchecked', function(event){
                if (event.type == 'ifChecked') {
                   ...
                   // $(this).parents().next('.subs-permissions').find('.subs-permission-checkbox').iCheck('check');
                }else{
                    $(this).parents().next('.subs-permissions').find('.subs-permission-checkbox').iCheck('uncheck');
                }
            })
            $(".subs-permission-checkbox").on('ifChecked ifUnchecked', function(event){
                if (event.type == 'ifChecked') {
                 //   $(this).parents('.sub-permissions').prev('.top-permission').find('.top-permission-checkbox').iCheck('check');
                    console.log($(this).parents('.subs-permissions').prev().find('.sub-permission-checkbox').length);
                    $(this).parents('.subs-permissions').prev().find('.sub-permission-checkbox').iCheck('check');
                }
            })
